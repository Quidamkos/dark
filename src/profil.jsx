...
function Profil() {
  ...
  const storedranks = JSON.parse(localStorage.getItem('ranks'));
  ...
  const [rank, setRank] = useState(storedranks ? storedranks.rank : []);

  const [level, setLevel] = useState(0);
  const [futurLevel, setFuturLevel] = useState(50);

  useEffect(() => {
    const settings = JSON.stringify({ birth, nickname, experience, money });
    const ranks = JSON.stringify({ rank });
  ...
  function experienceNeededForNextLevel(currentExperience, currentLevel) {
    ...
    const currentLevelExperience = experienceForLevel(currentLevel - 1);
  ...
  const expBarWidth = () => {
    ...
    const expIntoCurrentLevel = experience - experienceForLevel(level - 1);
  ...
  const currentLevelExperience = experience - experienceForLevel(level - 1);

  const [imageBase64, setImageBase64] = useState(
    localStorage.getItem('photoProfil') || personna
  );
          ...
          <div className='stats-mainPlayer stats'>
            {viewSettings ? (
              ...
              <div>
                ...
                />
                <button className='imgPlayer' onClick={triggerFileSelectPopup}>
                  Changer l'image
                </button>
              </div>
            )}
            ...
            {viewSettings ? (
              ...
              <div className='name'>
                <button className='deleteBtn' onClick={reset}>
